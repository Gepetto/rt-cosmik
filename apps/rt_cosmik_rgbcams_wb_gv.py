# To run the code : python3 apps/rt_cosmik_rgbcams_wb_gv.py cuda /root/workspace/mmdeploy/rtmpose-trt/rtmdet-nano /root/workspace/mmdeploy/rtmpose-trt/rtmpose-m
# or python3 -m apps.rt_cosmik_rgbcams_wb_gv cuda /root/workspace/mmdeploy/rtmpose-trt/rtmdet-nano /root/workspace/mmdeploy/rtmpose-trt/rtmpose-m
            ...
            if len(keypoints_list)!=2: #number of cams
                ...
                if len(keypoints_buffer) == 30:
                    ...
                    if first_sample:
                        ...
                        #Blue markers
                        for index in range(len(augmented_markers)):
                            viz.viewer.gui.addSphere('world/'+marker_names[index],0.01,[0,0,1,1])
                            M = pin.SE3(pin.SE3(Rquat(1, 0, 0, 0), np.matrix([augmented_markers[index][0],augmented_markers[index][1],augmented_markers[index][2]]).T))
                            place(viz,'world/'+marker_names[index],M)

                        #Red estimated markers
                        for index in range(len(augmented_markers)):
                            viz.viewer.gui.addSphere('world/estimated'+marker_names[index],0.01,[1,0,0,1])
                            M = human_data.oMf[human_model.getFrameId(marker_names[index])]
                            place(viz,'world/estimated'+marker_names[index],M)
