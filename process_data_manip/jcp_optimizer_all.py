...
# ----------------- main optimization -----------------
def parse_args():
    p = argparse.ArgumentParser(description="Per-JCP offset optimization across all STATIC subjects (global RMSE).")
    p.add_argument("--base-path", default="/home/msabbah/pinocchio-3x/src/rt-cosmik")
    p.add_argument("--save-offsets-json", default=None,
                   help="If given, save learned (20x3) offsets JSON to this path; otherwise write once in out-dir of the first subject.")
...
for subject_name in SUBJECT_NAMES:
    ...
    mocap_csv = base / f"output/{subject_name}/mocap/static/mocap_downsampled_to_40hz.csv"
    ...
    frame_indices = range(0, len(mks_dict), max(1, args.stride))
...
def objective(theta):
    ...
    for subj in subjects:
        jcp_m_full = subj.jcp_m[:30,:]  # (T_i, 60)
        meas_m = subj.meas_m[:30,:]     # (T_i, M*3)
        ...
        meas_all.append(meas_m)
        aug_all.append(augmented_array)

    # Concatenate across subjects (frames stack vertically, columns already match)
    meas_all = np.vstack(meas_all)
    aug_all  = np.vstack(aug_all)

    val = np.sqrt(np.mean(meas_all - aug_all)**2)
...
off_json = Path(args.save_offsets_json)
