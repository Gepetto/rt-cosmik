...
class SubjectData:
    ...
    mass: float
    gender: str
    jcp_m: np.ndarray          # (T_i, 60) meters
    meas_m: np.ndarray         # (T_i, M*3) meters, same markers order across subjects


# --------------- names ----------------
SUBJECT_NAMES = ['Alessandro', "Anais", "Anastasia", "Batiste", "Bilal", "Claire_", "Clement", "Emmanuelle", "Flavie", "Guilhem", "Herbert", "Kahina", "Marie_M", "Mathis", "Maxime_", "Mohamed", "Nicolas", "Zoe"]
...
def parse_args():
    p = argparse.ArgumentParser(description="Per-JCP offset verification across all STATIC subjects (global RMSE).")
    p.add_argument("--base-path", default="/home/msabbah/pinocchio-3x/src/rt-cosmik")
    p.add_argument("--load", default="/home/msabbah/pinocchio-3x/src/rt-cosmik/output/offset/optimized_offset_all.json",
                   help="If given, load learned (20x3) offsets JSON from this path.")
...
# Load once
warmed = loadModel(augmenterDir=args.augmenter_dir, augmenterModelName="LSTM",
                   augmenter_model=args.augmenter_model)

subjects: list[SubjectData] = []

for subject_name in SUBJECT_NAMES:
    info_path = base / f"output/{subject_name}/info.txt"
    height, mass, gender = read_subject_info(str(info_path))

    mocap_csv = base / f"output/{subject_name}/mocap/lower/mocap_downsampled_to_40hz.csv"
    if not mocap_csv.exists():
        raise FileNotFoundError(mocap_csv)

    df = pd.read_csv(mocap_csv)

    # Strip "<subject>:" prefixes if present in column names
    df.columns = [c.replace(f"{subject_name}:", "") for c in df.columns]

    # -> dict[frame_idx] -> dict[marker_name] -> np.array(3,), UNITS mm in your utils
    mks_dict, _ = read_mks_data(df, start_sample=0)

    jcp_rows = []
    meas_rows = []

    # stride within subject if needed
    frame_indices = range(0, len(mks_dict), max(1, 1))
    for k in frame_indices:
        markers_frame = mks_dict[k]  # dict[str] -> (3,) in mm
        jcp, seg_lengths, norms = compute_joint_centers_from_mks(markers_frame)  # returns GLOBAL jcp in mm, you convert below

        # (20,3) in meters
        jcp_row = np.stack(
            [np.asarray(jcp.get(name, np.full(3, np.nan))).reshape(3,) for name in JCP_NAMES],
            axis=0
        ) / 1000.0
        jcp_rows.append(jcp_row.reshape(-1))

        # measured markers (match the AUGMENTED/MOCAP subset after your DROP)
        meas = np.array([markers_frame[m] for m in MOCAP_MARKERS if m in markers_frame], dtype=float) / 1000.0
        meas_rows.append(meas.reshape(-1))
    ...
    subjects.append(
        SubjectData(
            ...
            mass=float(mass),
            gender=str(gender),
            jcp_m=np.asarray(jcp_rows, dtype=float),     # (T_i, 60)
            meas_m=np.asarray(meas_rows, dtype=float)    # (T_i, 3*len(MOCAP_MARKERS_kept))
        )
    )

# Full-resolution re-run with best theta (no stride)
_, _ , best_theta = load_offsets_json(args.load)


for subj in subjects:
    jcp_m_full = subj.jcp_m  # (T_i, 60)
    meas_m = subj.meas_m     # (T_i, M*3)
    ...
    for ii in range(jcp_m_full.shape[0]):
        ...
        if len(keypoints_buffer) == 30:
            ...
            augmented_markers = augmentTRC(keypoints_buffer_array, subject_mass=subj.mass, subject_height=subj.height, models = warmed,
    ...
    for jj in range(len(markers_lstm)):
        segments_frames = construct_segments_frames(markers_lstm[jj], with_hand=True, with_head=False, gender=subj.gender, subject_height=subj.height)
        segments_frames_list.append(segments_frames)

    jcp_off = apply_per_jcp_local_offsets(subj.jcp_m, segments_frames_list, best_theta)

    augmented_base_array = np.vstack(augmented_markers_list)

    print(f" RMSE without the offset for subject {subj.name} : {np.sqrt(np.mean((augmented_base_array - meas_m)**2)):.4f} m")
    ...
    print(f" RMSE with the offset for subject {subj.name} : {np.sqrt(np.mean((augmented_array - meas_m)**2)):.4f} m")

    
    input(f"Press Enter to start visualization for subject {subj.name}...")
    visualize_simple(
        preds_full=augmented_array,
        meas_m_full=meas_m,
        aug_markers_base=augmented_base_array,
        jcp_m_full=jcp_m_full,
        jcp_off_full=jcp_off,
    )