...
from utils.data_elaboration_utils import  init_kf_state, kf_keypoints_step, to_utc, fuse_stereo_keypoints, kabsch_global, estimate_joint_axis_accel_variances,build_per_joint_Q, make_virtual_shoulders, remap_hpe_to_mocap, plot_ref_est_ft_concatenated, mpjpe, score_to_rgba, plot_3D_keypoints_and_scores
...
csv_path = f"{base_path}/output/cosmik/cosmik_anais/{task}/q_cosmik_swika.csv"  # your file
...
csv_path = f"{base_path}/output/cosmik_jcp/{subject}/{task}_3d_keypoints.csv"   
...
K = len(bases)
N = len(df)
keypoints_02 = np.empty((N, K, 3), dtype=float)

for k, b in enumerate(bases):
    keypoints_02[:, k, 0] = df[f"{b}_x"].to_numpy(dtype=float)
    keypoints_02[:, k, 1] = df[f"{b}_y"].to_numpy(dtype=float)
    keypoints_02[:, k, 2] = df[f"{b}_z"].to_numpy(dtype=float)


csv_path = f"{base_path}/output/cosmik_jcp/{subject}/{task}_3d_keypoints46.csv"    
...
K = len(bases)
N = len(df)
keypoints_46 = np.empty((N, K, 3), dtype=float)

for k, b in enumerate(bases):
    keypoints_46[:, k, 0] = df[f"{b}_x"].to_numpy(dtype=float)
    keypoints_46[:, k, 1] = df[f"{b}_y"].to_numpy(dtype=float)
    keypoints_46[:, k, 2] = df[f"{b}_z"].to_numpy(dtype=float)
...
df0 = pd.read_csv(f"{base_path}/output/output_2d/{subject}/{task}/{task}_scores_0.csv")
df2 = pd.read_csv(f"{base_path}/output/output_2d/{subject}/{task}/{task}_scores_2.csv")
...
df4 = pd.read_csv(f"{base_path}/output/output_2d/{subject}/{task}/{task}_scores_4.csv")
df6 = pd.read_csv(f"{base_path}/output/output_2d/{subject}/{task}/{task}_scores_6.csv")
...
# csv_path ='/root/workspace/ros_ws/src/rt-cosmik/output/4279/mocap_jcp/robot_welding/4279joint_center_positions.csv'
df = pd.read_csv(csv_path)
# get base joint names in order of first appearance
bases = []
for c in df.columns:
...
# T_c0 = np.array([
#     [ 0.992028, -0.013888,  0.125248, -0.854942],
#     [-0.126016, -0.111799,  0.985708, -3.015875],
#     [ 0.000313, -0.993634, -0.112658,  1.096019],
#     [ 0.0     ,  0.0     ,  0.0     ,  1.0     ],
# ], dtype=float)
...
# T_c2 = np.array([
#     [ 0.982467,  0.029323, -0.184115, -0.033858],
#     [ 0.186121, -0.096903,  0.977737, -2.969303],
#     [ 0.010829, -0.994862, -0.100661,  1.108843],
#     [ 0.0     ,  0.0     ,  0.0     ,  1.0     ],
# ], dtype=float)
...
# T_c4 = np.array([
#     [-0.997925,  0.032119, -0.055802, -0.602563],
#     [ 0.057726,  0.062495, -0.996374,  2.315439],
#     [-0.028515, -0.997528, -0.064219,  1.106578],
#     [ 0.0     ,  0.0     ,  0.0     ,  1.0     ],
# ], dtype=float)
...
# T_c6 = np.array([
#     [-0.973170, -0.012497, -0.229749,  0.202322],
#     [ 0.227984,  0.082350, -0.970176,  2.312456],
#     [ 0.031044, -0.996525, -0.077292,  1.116433],
#     [ 0.0     ,  0.0     ,  0.0     ,  1.0     ],
# ], dtype=float)
...
vR_t, vL_t = make_virtual_shoulders(C7_t, RSHO_t, LSHO_t, up_offset=0.00, ap_offset=0.00)
 
jcp_hpe_filtered[:,HPE2_MOCAP["RShoulder"][1],:]=vR_t
jcp_hpe_filtered[:,HPE2_MOCAP["LShoulder"][1],:]=vL_t
    
# err=mpjpe(jcp, keypoints)      
# print(err)    
    
# err=mpjpe(jcp, jcp_hpe)      
# print(err)
...
# Save CSV
# df.to_csv(f'{base_path}/output/cosmik_jcp/{subject}/{task}_jcp_hpe_filtered.csv', index=False)
...
plot_ref_est_ft_concatenated(
    ...
    ft_in_ref_order=True,   # True if `ft` is already arranged in ref/mocap order
    title=None)
...
if param.viewer=="meshcat":
    ...
    images=[]
    #for i in range(i0,i0+2,step):#len(q_ref),step):#
