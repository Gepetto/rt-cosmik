...
if __name__ == "__main__":
    ...
    for subject in subjects:
        ...
        for trial in trials:
            if trial == "results" or trial == "cosmik_2cams" or trial == "output_2d":
                continue
            ...
            if num_of_cams == 2:
                P_cam = np.array(pd.read_csv(os.path.join(subject_path, "cosmik_2cams", trial, "augmented_markers_2.csv")).values).reshape(-1, 43,3)  # ou autre méthode de chargement
            elif num_of_cams == 4:
                P_cam = np.array(pd.read_csv(os.path.join(trial_path, "augmented_markers_4.csv")).values).reshape(-1, 43,3)  # ou autre méthode de chargement
            else:
                raise ValueError("num_of_cams must be 2 or 4")
