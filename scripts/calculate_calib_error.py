...
# # 1. Générer 43 points 3D aléatoires (repère 'source' P)
# np.random.seed(0)
# P = np.random.rand(43, 3)

# # 2. Créer une rotation 3D propre (R_true) et une translation aléatoire t_true
...
# # 3. Appliquer R_true,t_true pour générer l'autre nuage Q
# Q = (R_true @ P.T).T + t_true

# # 4. Centrer les nuages
# P_centroid = P.mean(axis=0)
# Q_centroid = Q.mean(axis=0)
# P_centered = P - P_centroid
# Q_centered = Q - Q_centroid

# # 5. Calculer la matrice de covariance H et sa SVD
# H = Q_centered.T @ P_centered
# U, S, Vt = np.linalg.svd(H)

# # 6. Calculer la rotation estimée R_est = U @ Vt (corriger pour det=+1 si nécessaire)
# R_est = U @ Vt
# if np.linalg.det(R_est) < 0:
#     U[:, -1] *= -1
#     R_est = U @ Vt

# # 7. Calculer la translation estimée t_est
# t_est = Q_centroid - R_est @ P_centroid

# # 8. Appliquer la transformation estimée sur P et évaluer l'erreur
# P_aligned = (R_est @ P.T).T + t_est
# errors = np.linalg.norm(P_aligned - Q, axis=1)
# print("Erreur maximale :", np.max(errors))
# print("Erreur RMS :", np.sqrt(np.mean(errors**2)))
