...
SUBJECTS = ["Alessandro","Bilal"]

TASKS = ["bolting","lifting","overhead","robot_sanding","robot_welding","sanding"]
...
def run_swika(no_trial: str, task: str):
    ...
    # input files
    path_to_csv = f"/root/workspace/ros_ws/src/rt-cosmik/output/{no_trial}/hybrik/{task}/hybrik_camera_0_mks_aligned.csv"
    ...
    csv_file = os.path.join(rt_cosmik_path,f"/root/workspace/ros_ws/src/rt-cosmik/output/{no_trial}/hybrik/{task}/mks_model_swika.csv") 
    ...
    csv_file = os.path.join(rt_cosmik_path, f"output/{no_trial}/hybrik/{task}/q_hybrik_swika.csv")
    df.to_csv(csv_file, index=False)
