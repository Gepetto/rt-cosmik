...
#         try: 
#             while not self.stop_event.is_set():
#                 with self.buffers['locks'][0]:
#                     cam_counters = list(self.buffers['counters'][:])

#                 if cam_counters == self.last_frame_counters:
#                     continue
#                 self.last_frame_counters = cam_counters.copy()

#                 with self.buffers['locks'][1]:
#                     kp_array = np.array(self.buffers['keypoints'][:]).reshape((26, 3))
#                 kpts_dict = dict(zip(self.keypoints_names, kp_array))

#                 with self.buffers['locks'][2]:
#                     mks_array = np.array(self.buffers['markers'][:]).reshape((43, 3))
#                 mks_dict = dict(zip(self.marker_names + ['Nose', 'Head', 'REar', 'LEar', 'REye', 'LEye'], mks_array))

#                 with self.buffers['locks'][3]:
#                     q = np.array(self.buffers['q'][:])
