...
#                                 self.buffers['counters'][0] = new_counters[0]
#                                 self.buffers['counters'][1] = new_counters[1]
                            
#                             kp_dict = dict(zip(self.keypoints_names, filtered_keypoints_buffer[-1]))
#                             flat_kp = np.hstack([kp_dict[key] for key in self.keypoints_names])
#                             with self.buffers['locks'][1]:
#                                 self.buffers['keypoints'][:] = flat_kp

#                             mks_dict = dict(zip(self.marker_names, augmented_markers))

#                             flat_mks = np.hstack([mks_dict[key] for key in self.marker_names])
#                             with self.buffers['locks'][2]:
#                                 self.buffers['markers'][:] = flat_mks
...
#                                 with self.buffers['locks'][3]:  # reuse the same lock for q
#                                     self.buffers['q'][:] = q
#                                 ik_class._q0 = q
...
#                                 with self.buffers['locks'][3]:  # same lock for q
#                                     self.buffers['q'][:] = q
#                             else : 
#                                 raise ValueError("Invalid ik type, should be sbs (sample by sample) or mhe (moving horizon estimation)")
