...
from human_model.model_utils import construct_segments_frames, get_local_segments_positions  # Imported segment frames function
...
def animate_all(viz, markers_list, marker_names, vertex_factor=1, delay=0.03):
    """
    ...
    local_segments_positions = get_local_segments_positions(initial_segments)
    for segment, position in local_segments_positions.items() :
        print(segment, np.linalg.norm(position))
...
if __name__ == '__main__':
    ...
    animate_all(viz, markers_list, marker_names, vertex_factor=1, delay=0.03)
