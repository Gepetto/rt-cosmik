...
def loadModel_simple(augmenterDir, augmenterModelName="LSTM",augmenter_model='v0.3', model_name_upper=None, model_name_lower=None):
    """
    Load and initialize LSTM models for different augmenter types.
    Parameters:
    -----------
    ...
    for idx_augm, augmenterModelType in enumerate(augmenterModelType_all):
        augmenterModelDir = os.path.join(augmenterDir, augmenterModelName, 
                                            augmenterModelType)
        if augmenterModelType == "{}_lower".format(augmenter_model):
            session = ort.InferenceSession(f"{augmenterModelDir}/{model_name_lower}.onnx", sess_options=so, providers=["CPUExecutionProvider"])
        else:
            session = ort.InferenceSession(f"{augmenterModelDir}/{model_name_upper}.onnx", sess_options=so, providers=["CPUExecutionProvider"])
        models[augmenterModelType] = session

    return models

def augmentTRC_simple(keypoints_buffer, subject_mass, subject_height,
               models, augmenterDir, augmenterModelName='LSTM', augmenter_model='v0.3', id_upper=None, id_lower=None):
    """
    Augments the given keypoints buffer using specified models and parameters.
    Parameters:
    -----------
    ...
    for augmenterModelType in augmenterModelType_all:
        ...
        #print(augmenterModelDir)
        if "lower" in augmenterModelType:
            pathMean = os.path.join(augmenterModelDir, "stats_streaming", f"mean_train_{id_lower}.npy")
            pathSTD = os.path.join(augmenterModelDir, "stats_streaming", f"std_train_{id_lower}.npy")
        else:
            pathMean = os.path.join(augmenterModelDir, "stats_streaming", f"mean_train_{id_upper}.npy")
            pathSTD = os.path.join(augmenterModelDir, "stats_streaming", f"std_train_{id_upper}.npy")
        #print(pathMean)
